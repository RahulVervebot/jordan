...
// 1) Add the helper inside or outside the file
...
const Footer = () => {
    const refreshToken = async (oldToken: string) => {
        try {
            const response = await fetch("https://jordan.agoxn.cloud/api/auth/refresh", {
                method: "GET",
                headers: {
            ...
            // If successful, store the new token info
            if (response.ok) {
                const data = await response.json();
                // Example of storing your new token and user data
                ...
                // e.g., log user out, show a message, etc.
    ...
    // 3) useEffect to check on mount (or whenever you like)
    useEffect(() => {
        const storedToken = localStorage.getItem("access_token");
        console.log('storedToken',storedToken);
        if (!storedToken) return; // If no token, skip

        // If it's expired, call the refresh endpoint
    ...
    return (
        <>
            <div id="footer" className='footer'>
                <div className="footer-main bg-surface-footer">
                    <div className="container">
                        <div className="content-footer py-[60px] flex justify-between flex-wrap gap-y-8">
                            <div className="company-infor basis-1/4 max-lg:basis-full pr-7">
                                <Link href={'/'} className="logo">
                                    <div className="heading4">Jordan</div>
                                </Link>
                        ...
                        <div className="footer-bottom py-3 flex items-center justify-between gap-5 max-lg:justify-center max-lg:flex-col border-t border-line">
                            <div className="left flex items-center gap-8">
                                <div className="copyright caption1 bg-surface-footer">©2025 Jordan. All Rights Reserved.</div>
                            ...
                            <div className="right flex items-center gap-2">
                                <div className="caption1 bg-surface-footer">Payment:</div>
